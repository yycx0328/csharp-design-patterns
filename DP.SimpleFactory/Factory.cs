...
namespace DP.SimpleFactory
{
    ...
    {
        public static readonly string COUNTRY = ConfigurationManager.AppSettings["Country"];
        ...
        /// </summary>
        /// <returns></returns>
        public ICountry CreateInstance()
        {
            string[] setting = COUNTRY.Split(',');
            // 通过给定程序集的长格式名称加载程序集
            Assembly assembly = Assembly.Load(setting[0]);
            // 使用区分大小写的搜索，从此程序集中查找指定的类型，然后使用系统激活器创建它的实例
            return (ICountry)assembly.CreateInstance(setting[1]);
        }
    }
}
